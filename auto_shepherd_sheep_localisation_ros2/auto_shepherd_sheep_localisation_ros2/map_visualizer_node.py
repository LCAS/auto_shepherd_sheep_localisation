...
class MapVisualizer(Node):
    def __init__(self):
        ...
        self.sheep_clusters = []   # [{'id': str, 'latitude': float, 'longitude': float, 'size': int}]
        ...
        self.flask_thread = threading.Thread(
            target=lambda: self.socketio.run(self.app, host='0.0.0.0', port=8080, debug=True, use_reloader=False)
        )
        ...
        with self.lock:
            ...
            for pose in msg.poses:
                ...
                if len(self.sheep_history[sheep_id]) > self.max_history_points:
                    self.sheep_history[sheep_id] = self.sheep_history[sheep_id][-self.max_history_points:]
        self._send_update()
    ...
    def _calculate_fov(self):
        ...
        with self.lock:
            if self.drone_gps is None or self.gimbal is None or self.drone_attitude is None:
                return
            ...
            for px, py in corners_px:
                ...
                corner_lat, corner_lon = self._pixel_to_gps(px, py, lat, lon, altitude, gimbal_pitch, absolute_yaw)
                if corner_lat is not None:
                    corner_coords.append([corner_lat, corner_lon])
            
            self.camera_fov_corners = corner_coords
    
    def _pixel_to_gps(self, px, py, drone_lat, drone_lon, altitude, gimbal_pitch, gimbal_yaw):
        ...
        try:
            ...
            lon_per_meter = 1.0 / (40008000.0 * math.cos(math.radians(drone_lat)) / 360.0)
